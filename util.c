...
char read_push_buttons(void) {
	...
	if(!(in&0b00100000)) // if only bit 6 is 0
	{
		return 6;	
	}
	if(!(in&0b00010000)) // if only bit 5 is zero
	{
		return 5;
	}
	if(!(in&0b00001000)) // if only bit 4 is zero
	{
		return 4;
	}
	if(!(in&0b00000100)) // if only bit 3 is zero
	{
		return 3;
	}
	if(!(in&0b00000010)) // if only bit 2 is zero
	{
		return 2;
	}
	if(!(in&0b00000001)) // if only bit 1 is zero
	{
		return 1;
	}
...
char read_shaft_encoder(void) {
	...
	static unsigned char old_value = 0xC0;		// initialize to in a groove
	
	// Function variables
	unsigned char new_value = PINC&0xC0;			// read pin C and mask push buttons
	char rotation = 0;

	// If the knob was in a groove before and is not now
	if (old_value == 0xC0) {			
		if (new_value == 0x40)		// if you're rotating CW out of the previous groove
			rotation = 1;
		if (new_value == 0x80)		// if you're rotating CCW out of the previous groove
			rotation = -1;
	}
...
void stepper_init(void) {
	...
	PORTE &= 0x8F;  //Initial postion (0b1000xxxx) PE4-PE7
	wait_ms(2);
...
void  move_stepper_motor_by_step(int num_steps, int direction) {
	...
	{
		for(int i = 0; i < num_steps; i++) // repeat turn for number of steps
		{
